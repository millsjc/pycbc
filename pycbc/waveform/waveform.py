...
get_td_waveform.__doc__ = get_td_waveform.__doc__.format(
    params=parameters.td_waveform_params.docstr(
        prefix="    ", include_label=False
    ).lstrip(" ")
)
...
get_fd_waveform.__doc__ = get_fd_waveform.__doc__.format(
    params=parameters.fd_waveform_params.docstr(
        prefix="    ", include_label=False
    ).lstrip(" ")
)
...
def orthogonalize_subdominant_harmonic(
    ...
    if (sigma_sub > 0) & (sigma_dom > 0):
        ...
        zeta = pycbc.filter.overlap_cplx(
            htilde_dom, htilde_sub, psd, f_lower, f_end, normalized=False
        )
        ...
        if abs(zeta) >= 1:
            ...
            htilde_sub_perp = htilde_sub / sigma_sub - zeta * htilde_dom / sigma_dom
...
def get_two_pol_waveform_filter(outplus, outcross, template, **kwargs):
    ...
    if input_params["approximant"] in fd_approximants(_scheme.mgr.state):
        ...
        if "taper" in input_params.keys() and input_params["taper"] is not None:
            hp = wfutils.taper_timeseries(hp, input_params["taper"], return_lal=False)
            hc = wfutils.taper_timeseries(hc, input_params["taper"], return_lal=False)
        # total duration of the waveform
